...
if ($step == 2 && isset($_POST['jenis_paket']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $nama_paket        = mysqli_real_escape_string($koneksi, $paket_pilihan);

        // 2. Cari id_paket berdasarkan nama_paket di tabel paket
        $row_paket = mysqli_fetch_assoc(mysqli_query($koneksi,
            "SELECT id_paket FROM paket WHERE nama_paket = '$nama_paket' LIMIT 1"
        ));

        if (!$row_paket) {
            $errors[] = "Paket tidak ditemukan di database. Hubungi admin.";
        } else {
            $id_paket = (int) $row_paket['id_paket'];

            // 3. Simpan ke tabel pendaftaran_pemasangan
            ...
            if ($q) {
                $success = true;
                // Bersihkan session alamat (session login tetap ada)
                unset($_SESSION['alamat_pemasangan']);
            } else {
...
// =====================
// DATA PAKET (nama harus SAMA PERSIS dengan nama_paket di tabel paket)
// =====================
$paket_basic = [
...
.alert-success { background:#e8f5e9; color:#2e7d32; font-size:15px; text-align:center; padding:20px; }
...
.form-footer { display:flex; justify-content:space-between; margin-top:24px; }
...
.btn-paket { padding:16px 40px; border-radius:12px; font-size:16px; font-weight:800; cursor:pointer; border:2px solid #1a73e8; transition:.2s; }
...
.harga { font-weight:700; }

/* INFO PELANGGAN */
.info-pelanggan {
    background:#eef5ff; border:1px solid #c5d9f7;
    border-radius:10px; padding:14px 18px;
    font-size:14px; margin-bottom:24px; color:#333;
}
...
<?php if ($success): ?>
    ...
    <div class="alert alert-success">
        ...
        <small>Status: <strong>Pending</strong> — Admin akan memverifikasi dan menghubungi Anda segera.</small>
    </div>
    <div style="text-align:center;margin-top:20px;">
        <a href="index.php" class="btn-lanjut" style="text-decoration:none;display:inline-block;">
            Kembali ke Homepage
        </a>
    </div>
...
<?php else: ?>
    ...
    <form method="POST">
        ...
        <p style="text-align:center;font-weight:700;color:#333;margin-bottom:4px;">Pilih Jenis Paket</p>
